...
static int try(const char *testCp) {
	...
	for(i=0; i < sizeof(asciiTries) / sizeof(asciiTries[0]); i++) {
		test = libiconv_open(asciiTries[i], testCp);
		if(test != (iconv_t) -1)
			break;
	}
	if(test == (iconv_t) -1)
...
doscp_t *cp_open(int codepage)
{
	...
	from = libiconv_open(wcharCp, dosCp);
	if(from == (iconv_t)-1) {
	...
	to   =  libiconv_open(dosCp, wcharCp);
	if(to == (iconv_t)-1) {
...
{
	libiconv_close(cp->to);
	libiconv_close(cp->from);
	free(cp);
}
...
int dos_to_wchar(doscp_t *cp, char *dos, wchar_t *wchar, size_t len)
{
	...
	wchar_t *dptr=wchar;
	r=libiconv(cp->from, &dos, &in_len, (char **)&dptr, &out_len);
	if(r < 0)
		return r;
	*dptr = L'\0';
	return dptr-wchar;
		      ...
		      size_t len, int *mangled)
{
	...
	while(in_len > 0) {
		r=libiconv(conv, (char**)&wchar, &in_len, &dptr, &out_len);
		if(r >= 0 || errno != EILSEQ) {
...
static void initialize_to_native(void)
{
	...
	strcpy(cp, li);
	strcat(cp, "//TRANSLIT");
	to_native = libiconv_open(cp, wcharCp);
	if(to_native == (iconv_t) -1)
		to_native = libiconv_open(li, wcharCp);
	if(to_native == (iconv_t) -1)
